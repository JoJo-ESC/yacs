from __future__ import annotations
  ...
  def _record_failed_attempt(key: str) -> None:                                     
      ...
      if state is None or now - float(state["first_failed_at"]) >                   
  ...
  def _is_locked(key: str) -> bool:
      ...
      now = monotonic()
      if now - float(state["first_failed_at"]) > FAILED_WINDOW_SECONDS and now >=
  float(state["locked_until"]):                                                     
          _FAILED_LOGIN_ATTEMPTS.pop(key, None)
          return False                                                              
  ...
  def log_user_in(credentials: dict, session: dict, client_ip: str | None = None):
      """Log a user in by verifying database credentials and writing session 
  state."""                                                                         
  ...
  "missing_credentials", "message": "Email and password are required."}             
  ...
  def log_user_out(session: dict):
      ...
      if "user" in session:
          session.clear()                                                           
          return {"success": True, "status": "success", "message": "Logout 
  successful."}                                                                     
      return {"success": False, "status": "error", "code": "no_active_session",
  ...
  def get_current_user_session(session: dict):
      ...
      if not session_user:                                                          
          return {"success": False, "status": "error", "code": "not_authenticated",
  "message": "No active session."}                                                  
  ...
  "message": "No active session."}
      ...
      try:
          ...
          if user is None:
              session.clear()                                                       
              return {"success": False, "status": "error", "code":
  "not_authenticated", "message": "No active session."}                             
