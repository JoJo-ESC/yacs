from models import SessionLocal
  ...
  def create_user(user_data: dict):
      ...
      try:
          ...
          if not email or "@" not in email:
              return {"success": False, "status": "error", "message": "Invalid email
   address."}                                                                       
  
          password = user_data.get("password", "")                                  
          if len(password) < 8:
              return {"success": False, "status": "error", "message": "Password must
   be at least 8 characters."}
                                                                                    
          existing = db.query(User).filter(User.email == email).first()             
          if existing is not None:
              return {"success": False, "status": "error", "message": "Email already
   in use."}           

          try:
              password_hash = hash_password(password)
          except ValueError:
              return {"success": False, "status": "error", "message": "Password is 
  too long."}                                                                       
          ...
          new_user = User(                                                          
              ...
              preferred_semester=user_data.get("preferred_semester", "Fall          
  2025").strip() or "Fall 2025",                                                    
              role="user",                                                          
          )            
          db.add(new_user)
          db.commit()
          db.refresh(new_user)                                                      
          return {"success": True, "status": "success", "message": "User created.",
  "user_id": new_user.id}                                                           
      finally:    
          db.close()                                                                
  ...
  def delete_current_user(user_id: int):
      ...
      try:                                                                          
          ...
          if user is None:                                                          
              return {"success": False, "status": "error", "message": "User not 
  found."}                                                                          
  ...
  found."}
          ...
          user.preferred_semester = preferred_semester.strip() or                   
  ...
  def update_user_profile(user_id: int, update_data: dict):
      ...
      try:
          ...
          if next_email and next_email != user.email:
              existing_user = db.query(User).filter(User.email ==
  next_email).first()
              if existing_user:
